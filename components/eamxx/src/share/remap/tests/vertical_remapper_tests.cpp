...
TEST_CASE ("log_linear_vertical_remapper") {
  ...
  auto create_pmid = [](const auto& grid, const Real qtop, const Real qbot, bool use_exp) {
    auto layout = grid->get_vertical_layout(true);  // midpoints (LEV tag)
  ...
  auto pmid_exp_src = create_pmid(src_grid, qtop_src, qbot_src, true);
  auto pmid_exp_tgt = create_pmid(tgt_grid, qtop_tgt, qbot_tgt, true);

  // Pressure coords for linear remapper: p_k = q_k (uniform spacing in q)
  auto pmid_lin_src = create_pmid(src_grid, qtop_src, qbot_src, false);
  auto pmid_lin_tgt = create_pmid(tgt_grid, qtop_tgt, qbot_tgt, false);
  ...
  auto src_loglin = create_field("s3d_m", src_grid, false, false, false, true, 1);
  auto tgt_loglin = create_field("s3d_m", tgt_grid, false, false, false, true, 1);
  auto src_lin    = create_field("s3d_m", src_grid, false, false, false, true, 1);
  auto tgt_lin    = create_field("s3d_m", tgt_grid, false, false, false, true, 1);
