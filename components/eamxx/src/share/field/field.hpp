...
public:
  ...
  // It is not too unfrequent to have to query the Field for the valid_mask field. So we provide shortcuts.
  // NOTE: the user can manually set other "mask" field in the field header via FieldHeader's extra data API.
  // These methods are for a pre-defined mask that signals where the data is valid (mask!=0 or garbage (mask=0).
